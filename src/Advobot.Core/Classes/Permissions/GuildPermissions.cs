...
namespace Advobot.Core.Classes.Permissions
{
	...
	public static class GuildPerms
	{
		...
		/// <returns></returns>
		public static GuildPerm GetByName(string name) => Permissions.FirstOrDefault(x => x.Name.CaseInsEquals(name));
		/// <summary>
		/// Returns the first <see cref="GuildPerm"/> to have the given value.
		/// </summary>
		/// <param name="value"></param>
		/// <returns></returns>
		public static GuildPerm GetByExactValue(ulong value) => Permissions.FirstOrDefault(x => (ulong)x.Value == value);
		/// <summary>
		/// Returns the first <see cref="GuildPerm"/> to not have its value ANDed together with the argument equal zero.
		/// </summary>
		/// <param name="value"></param>
		/// <returns></returns>
		public static GuildPerm GetByIncludedValue(ulong value) => Permissions.FirstOrDefault(x => ((ulong)x.Value & value) != 0);
		/// <summary>
		/// Returns the first <see cref="GuildPerm"/> to equal 1 shifted to the left with the passed in number.
		/// </summary>
		/// <param name="bit"></param>
		/// <returns></returns>
		public static GuildPerm GetByBit(int bit) => Permissions.FirstOrDefault(x => (ulong)x.Value == (1UL << bit));
		/// <summary>
		...
		public static string[] ConvertValueToNames(ulong value)
			=> ConvertToPermissions(value).Select(x => x.Name).ToArray();
		/// <summary>
