...
namespace Advobot.Standard.Responses
{
	public sealed class Misc : CommandResponses
	{
		...
		{
			var description =
				$"Type `{prefix}{nameof(Commands.Misc.Commands)}` for the list of commands.\n" +
				$"Type `{prefix}{nameof(Commands.Misc.Help)} [Command]` for help with a command.";
		...
		public static AdvobotResult GeneralCommandInfo(IReadOnlyCollection<string> categories, string prefix)
		{
			...
			{
				Title = "Categories",
				Description = Default.FormatInterpolated($"Type {prefix}{nameof(Commands.Misc.Commands)} [Category] for commands from a category.\n\n{categories}"),
			});
		}
