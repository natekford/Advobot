using Advobot.Modules;

using Discord;

using YACCS.Interactivity;
using YACCS.Results;

namespace Advobot.Interactivity;

/// <summary>
/// Determines if a message is from the source channel.
/// </summary>
public sealed class EnsureSourceChannelCriterion : ICriterion<IGuildContext, IMessage>
{
	/// <inheritdoc />
	public ValueTask<IResult> JudgeAsync(IGuildContext context, IMessage input)
		=> new(context.Channel.Id == input.Channel.Id ? Result.EmptySuccess : Result.EmptyFailure);
}