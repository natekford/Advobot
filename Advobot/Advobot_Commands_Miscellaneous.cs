...
namespace Advobot
{
	...
	namespace Miscellaneous
	{
		...
		[OtherRequirement(1U << (int)Precondition.BotOwner)]
		[DefaultEnabled(true)]
		public class GetBotDMs : ModuleBase
		{
			[Command("getbotdms")]
			public async Task Command([Optional, Remainder] string input)
			{
		...
		public class Test : ModuleBase
		{
			...
			{
				await Actions.SendChannelMessage(Context, "test1");
			}
			[Command("test")]
			[Alias("t")]
			public async Task Command(string input, IUser user)
			{
				await Actions.SendChannelMessage(Context, "test2");
			}
		}
	}}