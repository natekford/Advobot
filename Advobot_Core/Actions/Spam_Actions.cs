	...
	namespace Actions
	{
		public static class SpamActions
		{
			public static async Task HandleSpamPrevention(IGuildSettings guildSettings, IGuild guild, IUser author, IMessage msg, ITimersModule timers = null)
			{
				...
				if (spam)
				{
					var content = $"The user `{author.FormatUser()}` needs `{spamUser.VotesRequired - spamUser.UsersWhoHaveAlreadyVoted.Count}` votes to be kicked. Vote by mentioning them.";
