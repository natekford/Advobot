			...
			public static async Task SetChannel(MyCommandContext context, LogChannelType channelType, ITextChannel channel)
			{
				switch (channelType)
				{
					case LogChannelType.Server:
					{
						...
						{
							await MessageActions.MakeAndDeleteSecondaryMessage(context, String.Format("That channel is already the current {0} log.", channelType.EnumName().ToLower()));
							return;
						}
				...
				await MessageActions.MakeAndDeleteSecondaryMessage(context, String.Format("Successfully set the {0} log as `{1}`.", channelType.EnumName().ToLower(), channel.FormatChannel()));
			}
			public static async Task RemoveChannel(MyCommandContext context, LogChannelType channelType)
			{
				switch (channelType)
				{
					case LogChannelType.Server:
					{
						...
						{
							await MessageActions.MakeAndDeleteSecondaryMessage(context, String.Format("The {0} log is already off.", channelType.EnumName().ToLower()));
							return;
						}
				...
				await MessageActions.MakeAndDeleteSecondaryMessage(context, String.Format("Successfully removed the {0} log.", channelType.EnumName().ToLower()));
			}
		}
	}
}
